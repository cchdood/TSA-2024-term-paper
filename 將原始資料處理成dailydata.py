...
t0 = dt.strptime("2015-1-1", "%Y-%m-%d")
tN = dt.strptime("2015-12-31", "%Y-%m-%d")

for y in range(t0.year - 2012, t0.year - 2012 + 1):
...
for i in range((tN - t0).days + 1):
    ...
    if t in CNY[t.year - 2012]:
        gap = (t - t0).days
        day_i = gap
        D[day_i] = D[day_i] + 1 / len(CNY[t.year - 2012])
...
xpoints = np.array([t0 + dt0.timedelta(days=t) for t in range(T)])
ypoints = np.array(X)

plt.plot(xpoints, ypoints)
plt.axvline(x = CNY[t0.year - 2012][0], color = 'r', label = 'axvline - full height')
plt.axvline(x = CNY[t0.year - 2012][-1], color = 'r', label = 'axvline - full height')
plt.title("2015 Lunar Calendar Holiday - 2/18~2/23")
plt.show()
